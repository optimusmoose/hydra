...
void CMDParser::help()
{
    ...
    << "\n"
    << "\tdir <string>, if no files specified, directory is used to recursively search for JSON files that match pattern .json$ in directorys one level below dir" << "\n"
    << "\t\tdir is also used for the output file location -> dir/hydra_output.consensusXML" << "\n"
    << "\t\tDefault: ./" << "\n"
    << "\t\tExample: dir ~/path/to/directory" << "\n"
    << "\n"
    << "\tfile <string>, specifies a JSON file for processing" << "\n"
    << "\t\tExample: file file1.json file file2.json" << "\n"
    << "\n"
    << "\nExecution examples:" << "\n"
    << "\n"
    << "\t./Hydra dir ~/path/ file ~/path/A/file.json file ~/path/B/file.json mz_distance .1 rt_distance 25 cs_enforced 1" << "\n"
    << "\n"
    ;
}
